...
% ---------------------------------------------------------------------------
% Reach / PAR paper - NeurIPS format.
%
% Uses the official neurips_2026.sty shipped alongside this file (fetched
% and verified from a mirror of the official NeurIPS 2026 style files;
% unmodified). [final] is used since this is an attributed writeup, not an
% anonymous submission. `numbers, compress` is passed to natbib before
% loading the conference style, per that style file's own documented
% mechanism for customizing natbib - this gives the numeric bracketed
% citation style ("[3]") conventional in NeurIPS papers rather than natbib's
% own default author-year style.
%
% No LaTeX toolchain was available in the environment this was written in,
% so this file has NOT been compiled locally. It was checked instead with a
% script verifying brace balance, environment nesting, and that every
% \citep/\citet key has a matching \bibitem (and vice versa). Please compile
% once (e.g. on Overleaf) and fix any local package-version quirks before
% submitting anywhere. If you intend an actual NeurIPS submission rather
% than just the visual format, note the official template also requires a
% Paper Checklist appendix, which is not included here.
% ---------------------------------------------------------------------------
...
operation \citep{driess2023palme,brohan2023rt2}. We take a different,
...
continuing \citep{yao2023react,schick2023toolformer}. Reach generalizes this
...
SayCan \citep{ahn2022saycan} grounds an LLM's language-based planning in a
learned affordance function so that proposed actions are both plausible and
physically feasible; \citet{huang2022zeroshot} show an
LLM can propose actionable step sequences for embodied tasks with only a
translation step onto admissible actions. PaLM-E \citep{driess2023palme} and
RT-2 \citep{brohan2023rt2} go further and fold visual and physical grounding
...
ReAct \citep{yao2023react} interleaves reasoning traces with actions so an
LLM agent can call a tool, observe the result, and revise its plan;
Toolformer \citep{schick2023toolformer} shows a model can learn \emph{when}
to invoke a tool via self-supervision, and Reflexion \citep{shinn2023reflexion}
adds verbal self-critique after a failed attempt. Multi-agent frameworks
such as AutoGen \citep{wu2023autogen} and embodied, lifelong-learning agents
such as Voyager \citep{wang2023voyager} treat other agents or skill libraries
...
WebArena \citep{zhou2023webarena} and Mind2Web \citep{deng2023mind2web}
benchmark agents that must complete tasks by operating real web
interfaces, and CogAgent \citep{hong2023cogagent} targets GUI grounding
directly from screenshots. Commercial computer-use agents that perceive a
screen and drive the mouse and keyboard, such as
Anthropic's \citep{anthropic2024computeruse}, follow the same
...
Webots \citep{michel2004webots} and the Robot Operating
System \citep{quigley2009ros} are the standard tooling for developing and
testing robot software before physical deployment; MuJoCo \citep{todorov2012mujoco}
and Isaac Gym \citep{makoviychuk2021isaacgym} target high-throughput
physics for learning. Sim-to-real transfer typically relies on domain
randomization \citep{tobin2017domain,peng2018simtoreal} to close the gap
...
\citet{amodei2016concrete} catalog concrete failure
...
AI \citep{bai2022constitutional} pursues a complementary strategy of
...
\begin{tikzpicture}[
  node distance=6mm,
  box/.style={draw, rounded corners, align=center, minimum height=8mm, minimum width=40mm, font=\small},
...
(Claude or Gemini, following the ReAct pattern \citep{yao2023react}) for
...
benchmarks and systems \citep{zhou2023webarena,deng2023mind2web,hong2023cogagent,anthropic2024computeruse}.
...
robot in Webots \citep{michel2004webots}. PAR already supports a ROS~2
...
WebArena \citep{zhou2023webarena} or Mind2Web \citep{deng2023mind2web} to be
